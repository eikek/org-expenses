...
points. Expense items can have one category, but in contrast to org's
property ~CATEGORY~, an expense category is just the name of the
parent headline.

Then those items are gathered from some org files and org tables are
generated to create an overview.
